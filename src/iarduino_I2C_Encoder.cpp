...
bool	iarduino_I2C_Encoder::_begin		(void){																				//	Параметр:				отсутствует
...
int16_t	iarduino_I2C_Encoder::getPosition	(void){																				//	Параметр:				отсутствует.
			...
			//	Читаем текущую позицию вала энкодера:																			//
				...
				f = ((uint16_t)(data[1]&~ENC_RES_SIGN)<<8)|data[0]; if( data[1] & ENC_RES_SIGN ){ f*= -1; }						//	Объеденяем два прочитанных байта в одно число и добавляем знак.
			}																													//
		//	Возвращаем результат:																								//
			return f;																											//
}																																//

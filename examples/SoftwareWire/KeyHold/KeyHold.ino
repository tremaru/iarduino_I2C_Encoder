...
void setup(){                                                //
    ...
    enc.begin(&sWire);                                       //   Инициируем работу с энкодером, указав ссылку на объект для работы с шиной I2C на которой находится энкодер (по умолчанию &Wire).
                                                             ...
                                                             //
void loop(){                                                 //
//  bool f = enc.getButton(KEY_HOLD_05);                     //   Считываем флаг удержания кнопки дольше 0,5 сек в переменную f.
    bool f = enc.getButton(KEY_HOLD_10);                     //   Считываем флаг удержания кнопки дольше 1,0 сек в переменную f.
//  bool f = enc.getButton(KEY_HOLD_20);                     //   Считываем флаг удержания кнопки дольше 2,0 сек в переменную f.
